import { useDeletePost } from "@/hooks/mapelHook";
...
import FormMapel from "./FormMapel";
...
const CardList = (props: Props) => {
  ...
  const showFormEditMapel = useGlobalState((state) => state.showFormEditMapel);
  const {
    mutate: deleteMutate,
  ...
  const editData = (params: boolean) => {
    setGlobalArray("showFormEditMapel", Number(props.data?.id), params);
  };

  return (
    <div className="card-list-item">
      {showFormEditMapel[Number(props.data?.id)] ? (
        <FormMapel data={props.data} />
      ) : (
        <>
